...
<div class="wrapper">
 <div class="header">
    Gabage Collection Management Waste Report

</div>
<div id="maintable">
<div style="margin-top: -19px; margin-bottom: 21px;">
<a id="addd" href="reports.php" style="float: none;">Back</a>
</div>
<form action="waste.php" method="get">
...
<table id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			...
			<th width="16.66%">Collector </th>
			<th width="16.66%">Client </th>

			<th width="16.66%">Waste </th>
			<th width="16.66%">Pay Type</th>
			<th width="20%">Garbage Weight</th>
			<th width="20%">Date </th>
	...
	<tbody>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT * FROM wastecollect WHERE date BETWEEN :a AND :b");
			...
			<?php
				...
				$result = $db->prepare("SELECT * FROM wastecollect");
				$result->execute();
				for($i=0; $row = $result->fetch(); $i++){
			?>
			<tr class="record">
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(amountpaid) FROM wastecollect WHERE date BETWEEN :a AND :b");
				...
				$results = $db->prepare("SELECT sum(amountpaid) FROM wastecollect ");
